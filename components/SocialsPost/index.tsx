 ...
 * Notes:
 * - Icon names NOT translated (identifiers)
 * - Optional action handlers with capability checks
 * - Buffer logo treated as decorative (screen reader already has "Share with")
...
const SocialsPost = ({
  ...
  const canAddToQueue = typeof onAddToQueue === "function";
  const canShareNow = typeof onShareNow === "function";
  const canSchedulePost = typeof onSchedulePost === "function";

  const handleAddToQueue = useCallback(() => {
    if (!canAddToQueue) return;
    onAddToQueue();
  }, [canAddToQueue, onAddToQueue]);

  const handleShareNow = useCallback(() => {
    if (!canShareNow) return;
    onShareNow();
  }, [canShareNow, onShareNow]);

  const handleSchedulePost = useCallback(() => {
    if (!canSchedulePost) return;
    onSchedulePost();
  }, [canSchedulePost, onSchedulePost]);

  return (
    <section aria-labelledby="socials-post-title">
      <h2 id="socials-post-title" className="sr-only">
        {t("Social posts")}
      </h2>

      <p>
        {t("Here's an example of promotional content optimized for Twitter and Facebook:")}
      </p>
      ...
      <div className="mt-5">
        ...
        <div className="flex flex-wrap -mt-4 -ml-4 md:block md:ml-0">
          <button
            type="button"
            className="btn-dark btn-medium ml-4 mt-4 px-4 rounded-md md:w-full md:ml-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-1 focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
            onClick={handleAddToQueue}
            disabled={!canAddToQueue}
            aria-disabled={!canAddToQueue}
          >
          ...
          <button
            type="button"
            className="btn-white btn-medium ml-4 mt-4 px-4 rounded-md md:w-full md:ml-0 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-1 focus-visible:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed"
            onClick={handleShareNow}
            disabled={!canShareNow}
            aria-disabled={!canShareNow}
          >
            <span>{t("Share now")}</span>
            <Icon name="share-1" aria-hidden="true" />
          </button>
          ...
          <button
            ...
            aria-disabled={!canSchedulePost}
          >
            <span>{t("Schedule post")}</span>
