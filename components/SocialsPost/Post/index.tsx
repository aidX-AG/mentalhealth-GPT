...
const Post = ({
  item
}: PostProps) => <div className="flex mb-4 p-5 bg-n-1 rounded-xl last:mb-0 md:block dark:bg-n-6">
        ...
        <div className="w-[calc(100%-2.5rem)] pl-4 md:w-full md:pl-0 md:pt-4">
            <div>
                {item.content}{" "}
                <a className="underline text-primary-1 break-words" href={item.link} target="_blank" rel="noopener noreferrer">
                    {item.link}
                </a>
                {item.tags.map((tag: any, index: number) => <span className="text-primary-1" key={index}>
                        {" "}{t("#")}{tag}
                    </span>)}
            </div>
