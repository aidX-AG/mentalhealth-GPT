...
const Profile = ({
  visible
}: ProfileProps) => <div className={`${visible ? "mb-6" : "mb-3 shadow-[0_1.25rem_1.5rem_0_rgba(0,0,0,0.5)]"}`}>
            ...
            <div className={`flex items-center ${visible ? "justify-center" : "px-2.5 py-2.5 pb-4.5"}`}>
                ...
                {!visible && <>
                        <div className="ml-4 mr-4">
                            <div className="base2 font-semibold text-n-1">{t("Tran Mau Tri Tam")}</div>
                            <div className="caption1 font-semibold text-n-3/50">{t("tam@ui8.net")}</div>
                        </div>
                        <div className="shrnik-0 ml-auto self-start px-3 bg-primary-2 rounded-lg caption1 font-bold text-n-7">{t("Free")}</div>
                    </>}
            </div>
            {!visible && <Link className="btn-stroke-dark w-full mt-2" href="/pricing">{t("Upgraded to Pro")}</Link>}
        </div>
    </div>;
