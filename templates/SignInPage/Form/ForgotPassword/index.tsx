...
const ForgotPassword = ({ onClick, backLabel, emailPlaceholder, submitLabel }: ForgotPasswordProps) => {
  const [email, setEmail] = useState<string>("");

  return (
    <>
      <button className="group flex items-center mb-8 h5" onClick={onClick}>
        <Icon className="mr-4 transition-transform group-hover:-translate-x-1 dark:fill-n-1" name="arrow-prev" />
        {backLabel}
      </button>
