...
type PackageProps = {
  ...
  upgradeLabel: string;
  // ⬇️ NEU: statt t("CHF ") – vom Server/Seite liefern
  currencyPrefix?: string; // default "CHF "
};
...
const Package = ({
  ...
  <div
    className={twMerge(
      `flex basis-1/3 border-r-2 border-n-3 p-8 bg-n-1 first:rounded-l-3xl last:rounded-r-3xl last:border-none 2xl:px-6 lg:shrink-0 lg:basis-[18.5rem] dark:bg-n-7 dark:border-n-6`,
      item.popular &&
  ...
  >
    <div className="relative flex flex-col grow z-2">
      ...
      <div className="mb-2">
        ...
        {/* Slash ist sprachneutral → kein t() nötig */}
        <span className={twMerge(`h4 text-n-4/50 ${item.popular && "text-n-4"}`)}>
          /{plan ? perYearLabel : perMonthLabel}
        </span>
      </div>
